...
const Checkout = () => {
  ...
  // Sync user name to formData when user becomes available
  useEffect(() => {
  ...
  const saveAddresses = async (newAddresses) => {
    try {
      console.log("Saving addresses to backend:", newAddresses);
      const res = await api.post('/auth/addresses', { addresses: newAddresses });
